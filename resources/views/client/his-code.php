    ...
    <div class="grid grid-cols-8 gap-4">
     ...
     <div class="col-span-8 sm:col-span-5 md:col-span-6 lg:col-span-6 xl:col-span-6 px-2 md:px-0"> 
      <div class="v-bg w-full mb-2 px-2"> 
       <h2 class="v-title border-l-4 border-gray-800 px-3 select-none text-gray-800 text-xl md:text-2xl font-bold"> LỊCH SỬ MUA MÃ NGUỒN </h2> 
                            ...
                            <tbody class="text-sm font-semibold">
                                <?php $i=0; foreach ($TN->get_list("SELECT * FROM `tbl_his_code` WHERE `user_id` = '" . $getUser['id'] . "' ORDER BY `id` DESC") as $row) {?>
                                    ...
                                    <td class="text-sm text-gray-800 text-left px-1 py-1 border-b"><span
                                            class="badge badge-danger"><?=$row['create_date'];?></span></td>
                                    <td class="text-sm text-gray-800 text-left px-1 py-1 border-b"><a href="<?=nguyencoder_dec(getCode($row['product_id'], 'link_down'));?>" target="_blank" class="cursor-pointer border rounded text-center cursor-pointer border-red-500 hover:border-red-500 bg-red-500 transition duration-200 hover:bg-red-500 text-white uppercase inline-block font-semibold">Tải xuống</a></td> 
                                </tr>
                                <?php }?>
                                                            </tbody>
          </table> 
